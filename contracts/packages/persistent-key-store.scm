  ...
  (package
   ...
   (define-method persistent-key-store (get key . args)
     ...
     (let* ((_key (persistent-key-store-package::make-key key _prefix))
       ...
       (display "persistent-key-store:get ") (display key) (display _key) (display " : ")(display _val)(display "\n")
       (cond (_val (eval (string->expression _val)))
             ((pair? args) (car args))
             ((throw "key does not exist" key)))))

   (define-method persistent-key-store (set key value)
     (display "persistent-key-store:set ") (display key) (display ":")(display value)(display "\n")
     ...
     (let* ((serialized-list (serialize-instance value))
       ...
       (display "persistent-key-store:set ")(display _key)(display " ")(display serialized-string) (display "\n")
